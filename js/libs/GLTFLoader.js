...
THREE.GLTFLoader = (function () {
	...
	GLTFLoader.prototype = {
		...
		load: function (url, onLoad, onProgress, onError) {
			...
			var path = this.path !== undefined ? this.path : 'res/' + THREE.LoaderUtils.extractUrlBase(url);
			...
			if (
				...
				loader.load(url, callback, onProgress, _onError);
	...
	GLTFParser.prototype.loadBuffer = function (bufferIndex) {
		...
		return new Promise(function (resolve, reject) {
			...
			let url = resolveURL(bufferDef.uri, options.path);
			if (
				window.wx && 
				// wx.getFileSystemManager && 
				url && !url.match(/^http/) && !url.match(/^data:/)
			) {
				...
				// 	},
				// 	fail: res => {
				// 		reject(new Error('THREE.GLTFLoader: Failed to load buffer "' + bufferDef.uri + '". res:' + JSON.stringify(res)));
				// 	},
				// });
	...
	GLTFParser.prototype.loadMaterial = function (materialIndex) {
		...
		if (metallicRoughness.baseColorTexture !== undefined) {
			...
			if (Array.isArray(metallicRoughness.baseColorFactor)) {
				var array = metallicRoughness.baseColorFactor;
				materialParams.color = new THREE.Color().fromArray(array);
				materialParams.opacity = array[3];
			}
		}
		...
		return Promise.all(pending).then(function () {
			...
			console.log('loadMaterial fail:', err);
...

		{
			...
			// 			if ( material.specularMap ) material.specularMap.encoding = THREE.LinearEncoding;
